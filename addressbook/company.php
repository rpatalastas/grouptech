...
if ($view->permitted($hash['category'], 'add')) {
	echo '<li><a href="companyadd.php'.$view->parameter(array('folder'=>$_GET['folder'])).'">アドレス追加</a></li>';
}
...
<table class="content" cellspacing="0"><tr><td class="contentfolder">
	<?=$view->category($hash['folder'], 'addressbook', 'company.php')?>
</td><td>
	<table class="list" cellspacing="0">
		<tr><th><?=$pagination->sortby('addressbook_company', '会社名')?></th>
...
?>
		<tr><td><a href="companyview.php?id=<?=$row['id']?>"><?=$row['addressbook_company']?></a>&nbsp;</td>
